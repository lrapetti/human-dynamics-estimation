...
int main(int argc, char* argv[])
{
    ...
    iDynTree::Position linkPosition;
    ...
    if (visualizeWrenches)
    {
        ...
        {
            linkPosition = viz.modelViz("human").getWorldLinkTransform(wrenchSourceLinkIndices.at(vectorIndex)).getPosition();
            for (size_t i = 0; i < 3; i++)
            {
                force.setVal(i, forceScalingFactor * wrenchMeasuresVector->data()[6 * vectorIndex + i]);
            }
            viz.vectors().addVector(linkPosition, force);
        }
    }
    ...
    while(viz.run() && !isClosing)
    {
        ...
        if (visualizeWrenches)
        {
            ...
            for (size_t vectorIndex = 0; vectorIndex < wrenchSourceLinks.size(); vectorIndex++)
            {
                ...
                viz.vectors().updateVector(vectorIndex, linkPosition, force);
                std::cerr << "frame: " << wrenchSourceLinkIndices.at(vectorIndex) << std::endl;
                std::cerr << "position: " << linkPosition.toString() << std::endl;
                std::cerr << "force: " << force.toString() << std::endl;
            }
        }

        for (size_t idx = 0; idx < 66; idx++)
        {
            std::cerr << "transform: " << viz.modelViz("human").getWorldLinkTransform(idx).toString() << std::endl;
        }
